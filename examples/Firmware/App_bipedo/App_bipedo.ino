...
#define BLYNK_TEMPLATE_ID "TMPL2D55EBUtI"
#define BLYNK_TEMPLATE_NAME "Robot bipedo"
#define BLYNK_AUTH_TOKEN "Uyh8CfRk2muW4-B7nGydshBBRfm6ZFgY"
//-------------------------------------------------//
...
void setup() {
  ...
  Timer_Biped.attach_ms(1, program_Biped);
...
BLYNK_CONNECTED() {
  ...
  Blynk.syncVirtual(V0, V1, V2, V3, V4, V5, V6, V7, V8, V9, V10, V11, V12, V13, V14, V15);
}
//-------------------------------------------------//

//-------------------------------------------------//
// Función que se ejecuta cuando hay un cambio en el valor del pin virtual V0
BLYNK_WRITE(V0) {  // LED 1 en el lado derecho
...
BLYNK_WRITE(V1) {  // LED 2 en el lado izquierdo
...
BLYNK_WRITE(V8) {  // Sonidos musicales
...
void program_Biped() {
  //-------------------------------------------------//
  if (adelante) {
    ESP32Biped.walk_Forward(velocidad1);
    _walk_Forward = 1;
  } else {
    if (_walk_Forward) {
  ...
  //-------------------------------------------------//
  if (retroceder) {
    ESP32Biped.walk_Backward(velocidad1);
    _walk_Backward = 1;
  } else {
    if (_walk_Backward) {
      ESP32Biped.standby_Mode();
  ...
  //-------------------------------------------------//
  if (derecha) {
    ESP32Biped.turn_Right(velocidad2);
    _turn_Right = 1;
  } else {
    if (_turn_Right) {
      ESP32Biped.standby_Mode();
  ...
  //-------------------------------------------------//
  if (izquierda) {
    ESP32Biped.turn_Left(velocidad2);
    _turn_Left = 1;
  } else {
    if (_turn_Left) {
      ESP32Biped.standby_Mode();
...
void program_Bateria() {
  Blynk.virtualWrite(V14, ESP32Biped.battery(2.9729));
}
//-------------------------------------------------//
BLYNK_WRITE(V15) {
